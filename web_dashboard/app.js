...
let naviMap; // Leaflet map instance

function initMap() {
    // 건국대학교 (서울시 광진구 능동로 120) - 37.540762, 127.079463
    naviMap = L.map('navi-map', {
        center: [37.540762, 127.079463],
        zoom: 17,
        zoomControl: false,
    ...
    L.tileLayer('assets/tiles/{z}/{x}/{y}.png', {
        maxZoom: 19,
        minZoom: 16,
        attribution: '&copy; OSM (Offline)'
    }).addTo(naviMap);
    
    L.control.attribution({position: 'bottomright'}).addTo(naviMap);
...
function startGeolocationTracking() {
    ...
    const geoOptions = {
        enableHighAccuracy: true, // 모바일 기기 등에서 GPS를 우선적으로 사용
        maximumAge: 0,            // 항상 최신 위치 정보 요청
        timeout: 5000
    };

    // 위치가 바뀔 때마다 자동으로 콜백 실행
    geoWatchId = navigator.geolocation.watchPosition(
        (position) => {
            const lat = position.coords.latitude;
            const lng = position.coords.longitude;
            
            // 새로운 GPS 좌표로 지도를 부드럽게 이동시킴
            if (naviMap) {
                naviMap.panTo([lat, lng]);
            }
        },
        (error) => {
            console.warn(`GPS 위치 정보를 가져올 수 없습니다 (${error.code}): ${error.message}`);
        },
        geoOptions
    );
}
...
function renderFrame(frame) {
    ...
    // 2. Ego Diagnostics
    const speed = frame.ego_speed_mps || appState.egoSpeed;
...
function drawRadar(frame) {
    ...
    // 4. Draw Pedestrians, past tracks, and predicted paths
...
function animationLoop(timestamp) {
    if (appState.mode === "REPLAY" && appState.isPlaying) {
        ...
        appState.lastAnimTime = timestamp;
        if (appState.mode === "LIVE") {
            const frame = frameDataPlaceholder();
            if (frame) drawRadar(frame);
...
// Helper to handle live stream buffering
let lastLiveFrame = null;
function frameDataPlaceholder() {
...
// Monkey-patch renderFrame to store active live frames for draw loop
const originalRenderFrame = renderFrame;
renderFrame = function(frame) {
    if (appState.mode === "LIVE") {
        lastLiveFrame = frame;
        // Update safety config variables if streamed
        if (frame.ego_speed_mps) appState.egoSpeed = frame.ego_speed_mps;
    }
    originalRenderFrame(frame);
};
