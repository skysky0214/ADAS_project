...
def _load_points_from_payload(frame: FrameInput) -> np.ndarray:
    if "points" in frame.payload:
        ...
        raise ValueError("OpenPCDet DSVT adapter expects frame.payload['points'] or ['point_path']")
...
class OpenPCDetDSVTPerceptionModel(PerceptionModel):
    """OpenPCDet DSVT-Pillar adapter for SUSTech pedestrian/cyclist detection."""
