# Fail-safe 상태 publish 설계

## 목적

ADAS node가 UI에서 감시할 수 있는 시스템 상태를 주기적으로 publish한다.

이 기능의 1차 목표는 UI를 직접 구현하는 것이 아니라, UI가 구독할 수 있는
상태 topic을 ADAS pipeline에서 제공하는 것이다.

## 현재 구현 위치

- 상태 판단 로직: `app/control/failsafe.py`
- ROS2 publish 연결: `app/main.py`
...
이 방식은 UI와의 contract를 빠르게 검증하기 쉽고, 기존 ROS package 구조 변경을
최소화할 수 있다.
...
- `--status-topic`
  - 기본값: `/adas/system_status`
- `--status-publish-period-sec`
  - 기본값: `0.5`
- `--lidar-timeout-sec`
  - 기본값: `1.0`

UI heartbeat timeout 기준은 UI 쪽에서 정해야 한다. 초기 권장값은 `1.0 sec`이다.
...
- ADAS node와 LiDAR 입력이 모두 정상일 때: `OK`
- ADAS node는 살아 있지만 LiDAR 입력이 끊겼을 때: `SENSOR_TIMEOUT`
- ADAS node가 종료되었을 때: `/adas/system_status` message 중단
