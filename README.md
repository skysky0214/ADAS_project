...
- PointPillars checkpoint는 repo에 포함하지 않고 `app/perception/checkpoints/`에 별도로 배치한다.
- DSVT checkpoint는 DSVT를 실행할 때만 별도로 준비한다.
...
/home/gh/anaconda3/envs/pedestrian_gt/bin/python tools/perception/run_openpcdet_pointpillar_frame.py \
  ...
  --output-json artifacts/pointpillar_frame_000000.json \
  --output-csv artifacts/pointpillar_frame_000000.csv
```
...
/home/gh/anaconda3/envs/pedestrian_gt/bin/python app/main.py \
  ...
  --output-dir artifacts/live_pointpillar_tracking
```
...
/home/gh/anaconda3/envs/pedestrian_gt/bin/python app/main.py \
  ...
  --safety-radius 1.0 \
  --marker-frame hesai_lidar \
  --output-dir artifacts/live_pointpillar_prediction
```

예측 결과는 아래 파일로 저장된다.

```text
artifacts/live_pointpillar_prediction/predicted_trajectories.csv
artifacts/live_pointpillar_prediction/predicted_trajectories.json
artifacts/live_pointpillar_prediction/ttc_warnings.json
artifacts/live_pointpillar_prediction/latency.csv
```
